...
//        GATE_INTAKE_C, GATE_INTAKE_TO_SHOOT_C,GATE_INTAKE_TO_SHOOT_C_GO, GATE_SHOOT_C,
    ...
    protected void setupFSM() {
        ...
        fsm.onStateUpdate(State.START_TO_SHOOT, (current, timeSinceTransition) ->
                !movement.isBusy() && timeSinceTransition > 500? State.SHOOT_PRELOAD : null);
        ...
        fsm.onStateUpdate(State.INTAKE_TO_SHOOT_A, (current, timeSinceTransition )  -> !movement.isBusy()? State.SHOOT_A  : null);
        ...
        // collectFromHuman
        fsm.onStateEnter(State.GO_COLLECT_FROM_HUMAN_1, () -> {movement.followPath(paths.goPrepareCollectFromHuman);});
        ...
        fsm.onStateEnter(State.RETURN_TO_LAUNCH_1,() -> {
            ...
            intake.command(Intake.Command.REJECT);} );
        fsm.onStateUpdate (State.RETURN_TO_LAUNCH_1,  () ->  !movement.isBusy() ?  State.LAUNCH_1 : null );
        ...
        //collectFromHuman2
        fsm.onStateEnter(State.GO_COLLECT_FROM_HUMAN_2, () -> {movement.followPath(paths.goPrepareCollectFromHuman2);});
        ...
        fsm.onStateUpdate (State.RETURN_TO_LAUNCH_2,  () ->  !movement.isBusy() ?  State.LAUNCH_2 : null );
